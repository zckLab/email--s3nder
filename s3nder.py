...
def show_progress_bar(duration=2.0, width=40):
    """Animated green progress bar from 0% to 100%."""
    steps = 50
    for i in range(steps + 1):
        pct = int(i / steps * 100)
        filled = int(width * i / steps)
        bar = '█' * filled + '░' * (width - filled)
        sys.stdout.write(f'\r  \033[92m[{bar}] {pct}%\033[0m')
        sys.stdout.flush()
...
[0m
    ...
    for i, company in enumerate(companies):
        ...
        if send_email(company["email"], subject, body):
            print(ASCII_SUCCESS + f"   {loc('art_success_label', base_lang)}")
            counts["sent"] += 1
        else:
            print(ASCII_FAILED + f" {loc('art_failed_label', base_lang)}")
            counts["failed"] += 1
    
    print("\n" + "="*40)
    if counts["failed"] > 0 and counts["sent"] == 0:
        # Don't show "COMPLETED" when everything failed — it's confusing
        print(loc("failed_summary", base_lang, sent=counts["sent"], failed=counts["failed"]))
    else:
